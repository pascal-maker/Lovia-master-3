...
const createPostWithFirebase = (postDetails, appConfig) => {
  const { photoFile } = postDetails;

  const accountCreationTask = (postDetails) => {
    return new Promise((resolve, _reject) => {
      authAPI
        .postUpload(postDetails, appConfig.appIdentifier)
        .then(async (response) => {
          if (response.error) {
            resolve({ error: response.error });
          } else {
            ...
            // console.log("response.fid : ", response);
            if (photoFile) {
              storageAPI
                ...
                .then((response) => {
                  ...
                  if (response.error) {
                    ...
                    authAPI
                      .updatePostPhoto(user.fid, response.downloadURL)
                      .then((_result) => {
                        resolve({
                          user: {
                            ...user,
...
const createAccountWithEmailAndPassword = (userDetails, appConfig) => {
  const { photoFile, profilePictureURL } = userDetails;
