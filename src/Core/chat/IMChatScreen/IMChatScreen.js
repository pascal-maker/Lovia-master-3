...
const IMChatScreen = (props) => {
  ...
  const onProfileClick = () => {
    // alert("ok123");
    navigation.navigate("ProfileScreen", { id: currentUser.id });
  };
  const onProfileRec = () => {
    // alert("ok123");s
    console.log("participant", channel.otherParticipants[0].userID);
    navigation.navigate("ProfileScreen", {
      id: channel.otherParticipants[0].userID,
    });
  };
  const openGif = () => {
    startUploadGif(global.link);
    //alert(global.link);
  };
  ...
  const startUploadGif = (uploadData) => {
    // alert('uploadDataGif : '+uploadData);
    var Data = {
      height: 1280,
      mime: "gif",
      modificationDate: "1626361907000",
      path: uploadData,
      size: 201031,
      width: 960,
    };
  ...
  const removeUser = () => {
    ...
    reportingManager.removeUser(myID, otherUserID).then((response) => {
      if (!response.error) {
        navigation.goBack(null);
      }
    });
  ...
  return (
    <IMChat
      ...
      onLeave={onLeave}
      onUserRemovePress={onUserRemovePress}
