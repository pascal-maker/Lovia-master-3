...
function BottomInput(props) {
  ...
  const _onImageChange = useCallback(({ nativeEvent }) => {
    //console.log("nativeEvent : ", nativeEvent.linkUri);

    // if (Platform.OS == "ios") {
    //   alert("nativeEvent : " + JSON.stringify(nativeEvent.data.uri));
    //   global.link = nativeEvent.data.uri;
    //   openGif(nativeEvent.data.uri);
    // } else {
    global.link = nativeEvent.linkUri;
    openGif(nativeEvent.linkUri);
    // }
  }, []);
      ...
      <View style={styles.bottomContentContainer}>
        ...
        <View style={styles.inputBar}>
          ...
          <View style={styles.inputContainer}>
            ...
            <TouchableOpacity
              onPress={onRBsheetOpen}
              style={styles.micIconContainer}
            >
              <Image
                style={styles.micIcon2}
