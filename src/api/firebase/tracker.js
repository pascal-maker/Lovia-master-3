...
export default class SwipeTracker {
  ...
  subscribeIfNeeded = () => {
    ...
    if (!this.state.dating.didSubscribeToSwipes) {
      ...
      this.outboundSwipesDislikeUnsubscribe = firebaseSwipe.subscribeToOutboundSwipesDislike(
        userId,
        this.onOutboundSwipesDislikeUpdate
      );
    }
  };
  ...
  unsubscribe = () => {
    ...
    if (this.outboundSwipesDislikeUnsubscribe) {
      this.outboundSwipesDislikeUnsubscribe();
    }
  ...
  onOutboundSwipesDislikeUpdate = (outboundSwipes) => {
    this.outboundSwipesDislike = outboundSwipes;
    this.hydrateSwipes();
  };
  ...
  hydrateSwipes() {
    ...
    const outboundSwipesDislike = this.outboundSwipesDislike;
    ...
    if (
      ...
      this.outboundSwipes &&
      this.bannedUserIDs &&
      this.outboundSwipesDislike
    ) {
      // we received all the data we need - users, inbound requests, outbound requests
      const outboundUserIDs = {};
      const outboundUserIDsAll = {};

      const outboundDislikeUserIDs = [];

      outboundSwipes.forEach((swipe) => {
        if (swipe.type == "like") {
          outboundUserIDs[swipe.swipedProfile] = true;
        }
        outboundUserIDsAll[swipe.swipedProfile] = true;
      });

      outboundSwipesDislike.forEach((swipe) => {
        outboundDislikeUserIDs.push(swipe.swipedProfile);
      });

      console.log("outboundDislikeUserIDsxxx", outboundDislikeUserIDs);
      ...
      const swipes = hydratedUsers
        .filter((user) => outboundUserIDsAll[user.id] == true)
        .filter((swipe) => !bannedUserIDs.includes(swipe.id));
      // .filter((disliked) => !outboundDislikeUserIDs.includes(disliked.id));
      console.log("swipesyyy", swipes.length);
